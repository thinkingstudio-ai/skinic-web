    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#0a0a0f]/80 backdrop-blur-md">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-xs text-white/50 font-mono mt-0.5">Studio</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            ...
            >
              ...
              <span className="text-sm text-white/85 hover:text-white transition-colors">My Studio</span>
            </Link>
          ) : (
            <>
              <Link href="/signin" className="text-sm text-white/75 hover:text-white transition-colors px-3 py-2">
                Sign In
              </Link>
              <Link
      ...
      {open && (
        <div className="md:hidden border-t border-white/5 bg-[#0a0a0f] px-6 py-4 flex flex-col gap-4 text-sm text-white/75">
          ...
          {user ? (
            <Link href="/studio" onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg bg-white/10 text-white text-center font-medium">
              My Studio
            </Link>
          ) : (
            <>
              <Link href="/signin" onClick={() => setOpen(false)} className="hover:text-white">Sign In</Link>
              <Link href="/signup?product=studio" onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg bg-violet-600 text-white text-center font-medium">Get Started Free</Link>
            </>
          )}
        </div>
      )}
